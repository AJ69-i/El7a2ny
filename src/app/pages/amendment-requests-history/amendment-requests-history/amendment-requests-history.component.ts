...
@Component({
  selector: 'app-amendment-requests-history',
  templateUrl: './amendment-requests-history.component.html',
  styleUrls: ['./amendment-requests-history.component.scss']
})
export class AmendmentRequestsHistoryComponent implements OnInit , AfterViewInit {
...
AmendmentRequestsHistory() {
    ...
    this.DATA = [];
    this.ObjectNames = [];
    const starCountRef = ref(this.Database, 'deletedCarEditRequestUsers');
    ...
    if(Note.valid) {
      let index = this.DATA.findIndex (
        (obj)=> {return (obj.note == element.note && obj.id == element.id && obj.area == element.area && obj.carModel == element.carModel &&
          ...
          obj.status == element.status && obj.telephone == element.telephone && obj.vin == element.vin && obj.budge == element.budge &&
          obj.describition == obj.describition);});
